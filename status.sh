...
directories=("api" "iso-compliance-frontend" "scripts" "PM")
...
if [ -f "iso-compliance-frontend/package.json" ]; then
    print_success "Frontend package.json exists"
    
    if [ -d "iso-compliance-frontend/node_modules" ]; then
...
if [ -f "iso-compliance-frontend/frontend.log" ]; then
    log_size=$(du -h "iso-compliance-frontend/frontend.log" | cut -f1)
    print_success "Frontend log file exists ($log_size)"
    
    echo -e "${BLUE}Last 3 lines of Frontend log:${NC}"
    tail -3 iso-compliance-frontend/frontend.log | sed 's/^/  /'
else
