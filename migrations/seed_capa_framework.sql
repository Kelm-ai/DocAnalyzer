-- Migration: Seed CAPA Process Assessment Framework
-- Creates the CAPA framework and 13 process-step requirements.
-- Covers ISO 13485:2016, 21 CFR 211.192, EU MDR 2017/745, 21 CFR 4.4, ICH Q10.
-- Note: 21 CFR 820.100 removed per QMSR transition (effective Feb 2, 2026).
-- Device CAPA reqs now via ISO 13485 8.5.2/8.5.3 (IBR into Part 820).
-- Idempotent-safe: uses WHERE NOT EXISTS for framework, ON CONFLICT for requirements.

BEGIN;

-- ============================================================================
-- STEP 1: Insert CAPA Process Assessment framework (if not exists)
-- ============================================================================

INSERT INTO frameworks (id, name, slug, description, standard_reference, system_prompt, is_active, display_order)
SELECT
    '00000000-0000-0000-0000-000000000002'::uuid,
    'CAPA Process Assessment',
    'capa-process',
    'Evaluate CAPA SOPs and procedures against ISO 13485:2016, 21 CFR 211.192, EU MDR 2017/745, 21 CFR 4.4, and ICH Q10 requirements for corrective and preventive action systems.',
    'ISO 13485:2016, 21 CFR 211.192, EU MDR 2017/745',
    $master_prompt$You are an expert pharmaceutical and medical device CAPA (Corrective and Preventive Action) process assessor with deep knowledge of ISO 13485:2016, 21 CFR 211.192, EU MDR 2017/745, 21 CFR 4.4 (combination products), and ICH Q10. Review ONE DOCUMENT AT A TIME and judge whether it addresses specific requirements for an effective CAPA process as defined across these regulations.
...
('10000000-0000-0000-0000-000000000001'::uuid,
 'ISO 13485 8.4/8.5.2a/8.2.2, 211.192',
 'Data Collection & Analysis',
 $req$You are evaluating CAPA Process Step 1 -- "Data Collection & Analysis".
...
- Batch record review (211.192) is not explicitly called out as a CAPA data source for organizations handling drug products.$req$,
 1, 'process', '00000000-0000-0000-0000-000000000002'::uuid),

-- CAPA-02: CAPA Initiation & Triage
('10000000-0000-0000-0000-000000000002'::uuid,
 'ISO 13485 8.5.2/8.5.3b, MDR Art.10',
 'CAPA Initiation & Triage',
 $req$You are evaluating CAPA Process Step 2 -- "CAPA Initiation & Triage".
...
Examples of strong PASS evidence:
- A decision tree or flowchart showing triage logic: no action, correction only, or full CAPA.
...
- Proportionality assessment is implied but not structured as a formal step.$req$,
 2, 'process', '00000000-0000-0000-0000-000000000002'::uuid),

-- CAPA-03: Investigation & Root Cause Analysis
('10000000-0000-0000-0000-000000000003'::uuid,
 'ISO 13485 8.5.2b/8.5.3a, 211.192',
 'Investigation & Root Cause Analysis',
 $req$You are evaluating CAPA Process Step 3 -- "Investigation & Root Cause Analysis".
...
- The procedure does not explicitly prohibit "human error" as a sole root cause -- consider requiring identification of systemic contributing factors.$req$,
 3, 'process', '00000000-0000-0000-0000-000000000002'::uuid),

-- CAPA-04: Impact Assessment & Scope Extension
('10000000-0000-0000-0000-000000000004'::uuid,
 '211.192, 21 CFR 4.4(c)(2), MDR Art.83',
 'Impact Assessment & Scope Extension',
 $req$You are evaluating CAPA Process Step 4 -- "Impact Assessment & Scope Extension".
...
- Benefit-risk reassessment for safety-related CAPAs is not explicitly required.$req$,
 4, 'process', '00000000-0000-0000-0000-000000000002'::uuid),

-- CAPA-05: Action Planning
('10000000-0000-0000-0000-000000000005'::uuid,
 'ISO 13485 8.5.2d/8.5.3c',
 'Action Planning',
 $req$You are evaluating CAPA Process Step 5 -- "Action Planning".
...
- Risk assessment of proposed actions (to ensure they don't introduce new problems) is not required.$req$,
 5, 'process', '00000000-0000-0000-0000-000000000002'::uuid),

-- CAPA-06: Verification & Validation of Actions
('10000000-0000-0000-0000-000000000006'::uuid,
 'ISO 13485 8.5.2e',
 'Verification & Validation of Actions',
 $req$You are evaluating CAPA Process Step 6 -- "Verification & Validation of Actions".
...
- Process validation requirements triggered by CAPA-related process changes are not referenced.$req$,
 6, 'process', '00000000-0000-0000-0000-000000000002'::uuid),

-- CAPA-07: Implementation
('10000000-0000-0000-0000-000000000007'::uuid,
 'ISO 13485 8.5.2d/8.5.3d',
 'Implementation',
 $req$You are evaluating CAPA Process Step 7 -- "Implementation".
...
- Effective date tracking for implemented changes is not required.$req$,
 7, 'process', '00000000-0000-0000-0000-000000000002'::uuid),

-- CAPA-08: Effectiveness Verification
('10000000-0000-0000-0000-000000000008'::uuid,
 'ISO 13485 8.5.2f/8.5.3e',
 'Effectiveness Verification',
 $req$You are evaluating CAPA Process Step 8 -- "Effectiveness Verification".
...
- Independent Quality approval of effectiveness is not required.$req$,
 8, 'process', '00000000-0000-0000-0000-000000000002'::uuid),

-- CAPA-09: Closure & Documentation
('10000000-0000-0000-0000-000000000009'::uuid,
 'ISO 13485 8.5.2/8.5.3/4.2.5, 211.192',
 'Closure & Documentation',
 $req$You are evaluating CAPA Process Step 9 -- "Closure & Documentation".
...
- Traceability to source nonconformity is implied but not formally required.$req$,
 9, 'process', '00000000-0000-0000-0000-000000000002'::uuid),

-- CAPA-10: Communication & Dissemination
('10000000-0000-0000-0000-000000000010'::uuid,
 'ISO 13485 5.5.3, MDR Art.83',
 'Communication & Dissemination',
 $req$You are evaluating CAPA Process Step 10 -- "Communication & Dissemination".
...
- Interface with external reporting systems (e.g., EUDAMED, FDA MDR) is not referenced.$req$,
 10, 'process', '00000000-0000-0000-0000-000000000002'::uuid),

-- CAPA-11: Management Review & Trending
('10000000-0000-0000-0000-000000000011'::uuid,
 'ISO 13485 5.6.2, ICH Q10',
 'Management Review & Trending',
 $req$You are evaluating CAPA Process Step 11 -- "Management Review & Trending".
...
- Resource allocation review for the CAPA system is not included in management review inputs.$req$,
 11, 'process', '00000000-0000-0000-0000-000000000002'::uuid),

-- CAPA-12: Combination Product Considerations
('10000000-0000-0000-0000-000000000012'::uuid,
 '21 CFR 4.4 (amended per QMSR), FDA Combo CGMP, ICH Q10',
 'Combination Product Considerations',
 $req$You are evaluating CAPA Process Step 12 -- "Combination Product Considerations".
...
- Continual improvement linkage per the expanded 4.4(b)(1)(iv) scope (ISO 13485 8.5.1) is not referenced.$req$,
 12, 'process', '00000000-0000-0000-0000-000000000002'::uuid),

-- CAPA-13: Preventive Action - Proactive Elements
('10000000-0000-0000-0000-000000000013'::uuid,
 'ISO 13485 8.5.3, MDR Annex I 20.4',
 'Preventive Action - Proactive Elements',
 $req$You are evaluating CAPA Process Step 13 -- "Preventive Action - Proactive Elements".
...
- Trend data thresholds for triggering preventive action evaluation are not specified.$req$,
 13, 'process', '00000000-0000-0000-0000-000000000002'::uuid)
