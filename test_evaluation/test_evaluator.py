    ...
    "recommendations": ["Next action", ...]
        ...
        try:
            ...
            try:
                ...
                self.print_status(f"Evaluation complete: {result['status']} (confidence: {result['confidence']:.2f})", "SUCCESS")
                return result

            except json.JSONDecodeError as e:
                self.print_status(f"Failed to parse JSON response: {e}", "ERROR")
                return {
                    "requirement_id": requirement['id'],
                    "status": "ERROR",
                    "confidence": 0,
                    ...
                    "evidence": [],
                    "gaps": ["Invalid API response format"],
                    "recommendations": ["Review prompt format"],
    ...
    def print_results_table(self, results: List[Dict]):
        ...
        if not TABULATE_AVAILABLE:
            ...
            for result in results:
                ...
                print(f"{result['requirement_id']:20} {status_color}{result['status']:12}{Style.RESET_ALL} {result['confidence']:6.2f} {result.get('tokens_used', 0):8}")
            return
    ...
    def export_results_to_excel(self, summary: Dict, excel_path: Path) -> Path:
        ...
        for requirement in summary.get('requirements_results', []):
            ...
            requirements_sheet.append([
                ...
                f"{requirement.get('confidence', 0):.2f}",
